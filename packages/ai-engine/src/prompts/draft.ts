...
const STATE_FRAMING: Record<TLeadStatus, string> = {
  ...
  no_show:
    'The lead booked a call and did not show up. THE LEAD HAS NOT SENT A MESSAGE explaining the absence — do not open as if you are replying to one. Open with understanding (life happens), then offer one clear path to rebook. Firm, not punishing. No guilt-trip, no desperation. If coach notes describe a pattern or context (first-time no-show vs repeat), honor that.',
  call_completed:
